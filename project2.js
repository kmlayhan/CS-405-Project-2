...
this.normalbuffer = gl.createBuffer();
   ...
   if (this.enableLight) {
        ...
        gl.uniform3fv(this.lightPosLoc, [lightX, lightY, 1]); // Use transformed light position
        gl.uniform1f(this.ambientLoc, this.ambient);
		
} else {
    gl.uniform1i(this.showTexLoc, true); // Ensure texture is shown without lighting
    gl.uniform1i(this.enableLightLoc, false); // Disable lighting in shader
}
	...
	enableLighting(show) {
		 ...
		 */
		gl.useProgram(this.prog);
		gl.uniform1i(this.showTexLoc, show ? 1 : 0); // Update to use 1 or 0 for boolean
    ...
    void main()
    {
        if (showTex) {
            if (enableLighting) { 
                ...
                gl_FragColor = texture2D(tex, v_texCoord); // Show texture without lighting
            }
        } else {
            gl_FragColor = vec4(1.0, 0, 0, 1.0); // Display red color if no texture
        }
    }
`;
